...
// Called once at server startup. Admin status is granted only via
// ADMIN_USERNAME/ADMIN_EMAIL/ADMIN_PASSWORD in backend/.env (see
// .env.example) — not automatically to whoever signs up first. If the
// account doesn't exist yet it's created as admin with that password; if it
// already exists it's just promoted to admin, without touching whatever
// password is already set (so restarting the backend — which happens a lot
// under nodemon — can never silently reset a password someone already
// changed through the app).
async function ensureAdminFromEnv() {
  const { ADMIN_USERNAME, ADMIN_EMAIL, ADMIN_PASSWORD } = process.env;
  if (!ADMIN_USERNAME && !ADMIN_EMAIL && !ADMIN_PASSWORD) {
    return; // no admin seeding requested
  }
  if (!ADMIN_USERNAME || !ADMIN_EMAIL || !ADMIN_PASSWORD) {
